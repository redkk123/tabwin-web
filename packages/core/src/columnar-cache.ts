...
/**
 * Quantos campos a projeção carrega além dos que o plano pede.
 *
 * O número vem de medição, não de gosto. Decodificar campos a mais numa
 * passada custa, e o custo não é linear (DOINF23, 32 mil registros, mínimo de
 * quatro rodadas intercaladas):
 *
 *     1 campo    181 ms   1,00x
 *     4 campos   192 ms   1,06x
 *    12 campos   242 ms   1,33x
 *    40 campos   500 ms   2,75x
 *    87 (todos) 1201 ms   6,62x
 *
 * Até uma dúzia o acréscimo é pequeno; depois dispara. Doze é onde a conta
 * ainda fecha: paga-se um terço a mais na primeira tabela para que as onze
 * seguintes saiam do cache, em vez de custarem uma passada inteira cada uma.
 */
export const PROJECTION_FIELD_BUDGET = 12;

/**
 * Escolhe quais campos a projeção vai guardar.
 *
 * Os do plano entram sempre — são o que se está tabulando agora. As vagas que
 * sobram vão para os campos **mais estreitos**, e a razão é que largura é um
 * bom indício de campo categórico: `SEXO`, `RACACOR` e `LOCNASC` ocupam um
 * caractere, enquanto `CODESTAB`, `CODMUNNASC` e as datas ocupam de seis a
 * oito. Ninguém tabula por data de nascimento; quase todo mundo tabula por
 * sexo. Empate é desfeito pelo nome, para a escolha não depender da ordem em
 * que os campos aparecem no arquivo.
 */
export function chooseProjectionFields(
  planFields: readonly string[],
  available: readonly { name: string; length: number }[],
  budget = PROJECTION_FIELD_BUDGET,
): string[] {
  // Normaliza sem validar: `normalizedFields` recusa lista vazia, e recusar
  // aqui derrubaria a tabulação por causa de uma otimização de cache. A
  // validação acontece no retorno, quando já há campos.
  const doPlano = [...new Set(planFields.map((campo) => campo.trim()).filter(Boolean))]
    .sort((a, b) => a.localeCompare(b));
  if (doPlano.length >= budget) return doPlano;

  const jaEscolhido = new Set(doPlano);
  const candidatos = available
    .filter((campo) => campo.name && !jaEscolhido.has(campo.name))
    .sort((a, b) => a.length - b.length || a.name.localeCompare(b.name))
    .slice(0, budget - doPlano.length)
    .map((campo) => campo.name);

  return normalizedFields([...doPlano, ...candidatos]);
}
